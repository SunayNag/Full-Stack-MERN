  ...
  return (
    <div className="p-4 mt-8 flex flex-col items-center justify-self-center min-h-10 bg-gray-200 w-150 rounded-xl ">
      <input
        ...
        className="border p-2 rounded w-7/10 text-center"
        placeholder="Type something..."
        style={{
          border: "solid black 1px",
        }}
      />
      <p className="mt-2 p-2 text-gray-500 border w-7/10 rounded text-center">
        You typed: {text}
      </p>
    </div>
  );
...
function UserCard({ name, email }) {
  ...
  return (
    <div className="p-4 mt-8 flex flex-col items-center justify-self-center min-h-10 bg-[#3D8D7A] w-150 border rounded-xl">
      <div className="px-4 py-2 text-2xl text-[#FBFFE4]">{name}</div>
      <div className="px-4 py-2 text-l text-[#FBFFE4]">{email}</div>
    </div>
  );
}
  ...
  return (
    <div className="p-4 mt-8 flex flex-col items-center justify-self-center min-h-10 bg-gray-200 w-150 rounded-xl ">
      <form onSubmit={handleSubmit}>
        <div className="my-4">
          <label className="text-lg">Enter your mail:</label>
          <br />
          <input
            type="text"
            value={email}
